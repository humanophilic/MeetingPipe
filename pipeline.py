import os, sys, warnings, pickle, cv2, time, torch
from re import I
...
def mp_detection(id, vfile, gpu_id):
    ...
    score_smile = smile_main(vfile, gpu_id)
    t8 = time.time()
    _tsmile.append(round((t8-t7), 2))

    n_frame = min(len(score_speak), len(score_nod), len(score_smile))
    score_speak = score_speak[: n_frame]
    score_nod = score_nod[: n_frame]
    score_smile = score_smile[: n_frame]

    speaks['{}'.format(id)] = score_speak
    nods['{}'.format(id)] = score_nod
    smiles['{}'.format(id)] = score_smile
...
def main():
    ...
    if args.gpus>1:
        ...
        for id, vfile in tqdm(enumerate(flist), total=args.nGuest):
            ...
            t7 = time.time()
            _tnod.append(round((t7-t6), 2))
            score_smile = smile_main(vfile, 0)
            t8 = time.time()
            _tsmile.append(round((t8-t7), 2))
